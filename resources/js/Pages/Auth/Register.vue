    ...
    <AuthenticationCard>
        ...
        <form @submit.prevent="submit">
            ...
            <div v-if="$page.props.jetstream.hasTermsAndPrivacyPolicyFeature" class="mt-4">
                <label for="terms" class="flex items-start gap-2">
                    ...
                    <span class="text-sm text-gray-600">
                        {{ t('auth.agree_to') }}
                        <a target="_blank" :href="route('terms.show')" class="font-medium text-gray-700 hover:text-primary-600 dark:text-gray-300 dark:hover:text-primary-400">{{ t('auth.terms') }}</a>
                        {{ t('common.and') }}
                        <a target="_blank" :href="route('policy.show')" class="font-medium text-gray-700 hover:text-primary-600 dark:text-gray-300 dark:hover:text-primary-400">{{ t('auth.privacy') }}</a>
                    </span>
                </label>
            ...
            <p class="mt-4 text-center text-sm text-gray-600">
                {{ t('auth.has_account') }}
                <Link :href="route('login')" class="font-semibold text-gray-700 hover:text-primary-600 dark:text-gray-300 dark:hover:text-primary-400">
                    {{ t('nav.login') }}
                </Link>
            </p>
        </form>
