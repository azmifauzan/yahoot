        ...
        <template #header>
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    >
                        <Icon name="play" class="h-4 w-4" /> {{ t('game.play_again') }}
                    </button>
                    <Link
                    ...
                    <a
                        :href="route('game.report', gameSession.id)"
                        target="_blank"
                        rel="noopener"
                        class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm border border-gray-200 transition hover:bg-gray-50 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-800 dark:hover:bg-gray-800"
                    >
                        <Icon name="document" class="h-4 w-4" /> {{ t('host.download_pdf') }}
                    </a>
                    <a
                        :href="route('game.export', gameSession.id)"
                        class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm border border-gray-200 transition hover:bg-gray-50 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-800 dark:hover:bg-gray-800"
                    >
                        <Icon name="download" class="h-4 w-4" /> {{ t('host.download_csv') }}
                    </a>
                </div>
            </div>
        </template>
        ...
        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
                    <div class="overflow-hidden rounded-[1.4rem] border border-white bg-white/90 p-5 shadow-lg shadow-primary-950/5 dark:border-white/10 dark:bg-gray-900/90">
                        <Icon name="users" class="mb-4 h-6 w-6 text-primary-600 dark:text-primary-400" />
                    ...
                    <div class="overflow-hidden rounded-[1.4rem] border border-white bg-white/90 p-5 shadow-lg shadow-primary-950/5 dark:border-white/10 dark:bg-gray-900/90">
                        <Icon name="document" class="mb-4 h-6 w-6 text-accent-blue" />
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <!-- Leaderboard Table -->
                    <div class="flex flex-col overflow-hidden rounded-[1.4rem] border border-white bg-white/90 shadow-lg shadow-primary-950/5 dark:border-white/10 dark:bg-gray-900/90 lg:col-span-2">
                                ...
                                <tbody class="divide-y divide-gray-100 dark:divide-gray-800 text-sm">
                                    <tr v-for="entry in leaderboard" :key="entry.player_id" class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                                        <td class="py-3 px-4 text-center font-extrabold text-gray-950 dark:text-white">
                                            <span class="inline-flex h-8 min-w-8 items-center justify-center rounded-full" :class="entry.rank <= 3 ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' : ''">{{ entry.rank }}</span>
                                        </td>
                                        <td class="py-3 px-4">
                                        ...
                                        <td class="py-3 px-4 text-center text-gray-700 dark:text-gray-300">
                                            <span class="inline-flex items-center gap-1"><Icon name="bolt" class="h-4 w-4 text-amber-500" /> {{ entry.best_streak ?? '-' }}</span>
                                        </td>
                    ...
                    <!-- Question Breakdown -->
                    <div class="flex flex-col overflow-hidden rounded-[1.4rem] border border-white bg-white/90 shadow-lg shadow-primary-950/5 dark:border-white/10 dark:bg-gray-900/90">
                        <div class="p-5 border-b border-gray-100 dark:border-gray-800">
                            ...
                            <div v-for="(q, idx) in questions" :key="q.id" class="pt-4 first:pt-0">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <div class="flex items-center justify-between text-[10px] text-gray-500 dark:text-gray-400 pt-1">
                                        <span class="flex items-center gap-1 font-semibold text-green-600 dark:text-green-400"><Icon name="check" class="h-3 w-3" /> {{ q.correct_count }} {{ t('host.correct') }}</span>
                                        <span class="flex items-center gap-1 font-semibold text-red-500 dark:text-red-400"><Icon name="x" class="h-3 w-3" /> {{ q.incorrect_count }} {{ t('host.wrong') }}</span>
                                        <span><span class="mr-1 inline-block h-1.5 w-1.5 rounded-full bg-current"></span>{{ q.no_answer_count }} {{ t('host.no_answer') || 'No answer' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
